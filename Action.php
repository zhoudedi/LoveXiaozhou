...
class LoveXiaozhou_Action extends Widget_Abstract_Users implements Widget_Interface_Do
{
    /**
     * 插件配置
     *
    ...
    public function __construct($request, $response, $params = NULL)
    {
        ...
        // 获取插件配置信息
        $this->_plugin = $this->options->plugin('LoveXiaozhou');
    }
    ...
    {
        // 创建表单
        $form = new Typecho_Widget_Helper_Form($this->security->getIndex('action/LoveXiaozhou'), Typecho_Widget_Helper_Form::POST_METHOD);
        // 邮箱地址
    ...
    private function resetForm($uid = 0)
    {
        ...
        $password->addRule('minLength', _t('为了保证账户安全, 请设置最少6位数的密码'), 6);
    ...
    private function doForget()
    {
        ...
        // 生成链接地址
        $uri = Typecho_Common::url('/action/LoveXiaozhou?' . http_build_query($query), $this->options->index);
        // 请求参数
        $data = array(
        ...
        switch ( $this->_plugin->public_interface ) {
            case 'sendcloud': // Send Cloud
                ...
                // 是否成功
                if ( !LoveXiaozhou_Plugin::sendCloud($data) ) {
                    // 输出错误信息
            ...
            case 'aliyun': // 阿里云
                ...
                // 是否成功
                if ( !LoveXiaozhou_Plugin::aliyun($data) ) {
                    // 输出错误信息
                    $this->widget('Widget_Notice')->set(_t('邮件发送失败, 请联系管理员解决!'), 'error');
                    // 跳转回去
                    $this->response->goBack();
            ...
            default: // SMTP
                ...
                // 是否成功
                if ( !LoveXiaozhou_Plugin::smtp($data) ) {
                    // 输出错误信息
                    $this->widget('Widget_Notice')->set(_t('邮件发送失败, 请联系管理员解决!'), 'error');
                    // 跳转回去
                    $this->response->goBack();
    ...
    private function reset()
    {
        ...
        if ( time() > $expire ) {
            ...
            // 跳转到找回密码界面
            $this->response->redirect(Typecho_Common::url('/action/LoveXiaozhou?forget', $this->options->index));
        }
        // 查询用户数据
