<?php
/**
 * Typecho 评论、回复、审核通过通知、发送给访客版、加找回密码插件
 *
 * @package LoveXiaozhou
 * @author  小周
 * @version 1.0.0
 * @link    https://www.lvxin.xn--6qq986b3xl/
 */
...
class LoveXiaozhou_Plugin implements Typecho_Plugin_Interface
{
    /**
     * 激活插件
     *
    ...
    static public function activate()
    {
        ...
        // 找回密码动作添加
        Helper::addAction('LoveXiaozhou', 'LoveXiaozhou_Action');
        // 找回密码路由添加
        Helper::addRoute('LoveXiaozhou_forget', '/LoveXiaozhou/forget', 'LoveXiaozhou_Action', 'forget');
        Helper::addRoute('LoveXiaozhou_reset', '/LoveXiaozhou/reset', 'LoveXiaozhou_Action', 'reset');
    }
    ...
    {
        // 删除动作
        Helper::removeAction('LoveXiaozhou');
        // 删除路由
        Helper::removeRoute('LoveXiaozhou_forget');
        Helper::removeRoute('LoveXiaozhou_reset');
    }
    ...
    static public function configCheck(array $settings)
    {
        ...
        // 获取插件配置
        $plugin = $options->plugin('LoveXiaozhou');
        // 是否启用配置验证
    ...
    static public function sendMail($commentId, $isApproved = FALSE)
    {
        ...
        if ( $isApproved ) {
            ...
            if ( !is_null($parentComment) ) {
                ...
                // 标题
                $data['subject'] = _t('收到您在 [' . trim($options->title) . ']  发表的评论！');
                // 读取模板
    ...
    static public function sendCloud($data)
    {
        ...
        $flag = TRUE;
        // 获取插件配置
        $plugin = Helper::options()->plugin('LoveXiaozhou');
        // 如果开启了Debug
        ...
        if ( in_array('enable', $plugin->public_forget) ) {
            ...
            if ( preg_match('/\/login\.php/i', $pathinfo) ) {
                ...
                <script>
                    ...
                    forget.href = '<?php echo Typecho_Common::url('/action/LoveXiaozhou?forget', Helper::options()->index);?>';
